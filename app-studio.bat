...
:build_twigkit
  CALL "%MVNCMD%" clean package -Pproduction --settings "%SETTINGS%"
EXIT /B 0
...
:launch_twigkit
  ...
  IF %FG%==1 (
	...
	@echo on
    CALL %MVNCMD% jetty:run -Pdevelopment --settings %SETTINGS% %TWIGKIT_OPTS% -Dtwigkit.http.port=%TWIGKIT_PORT% -Dtwigkit.http.stop.port=%STOP_PORT% -Dtwigkit.conf.overlay=file://src/dev/resources/conf -Dtwigkit.conf.watch=false
	@REM removed live config reload (-Dtwigkit.conf.watch=true) due to bug when used on Windows
  ) ELSE (
    @REM run in the background
    start /b cmd /c CALL %MVNCMD% jetty:run -Pdevelopment --settings %SETTINGS% %TWIGKIT_OPTS% -Dtwigkit.http.port=%TWIGKIT_PORT% -Dtwigkit.http.stop.port=%STOP_PORT% -Dtwigkit.conf.watch=false -Dtwigkit.conf.overlay=file://src/dev/resources/conf %SOLR_OPTS%>"%TWIGKIT_LOGS_DIR%/app-studio-%TWIGKIT_PORT%-console.log" 2>&1
	@REM removed live config reload (-Dtwigkit.conf.watch=true) due to bug when used on Windows
